...
# GECCO preset for the new local-window dual-view route:
#   X_t: [B, M, L] -> H1_t/H2_t: [B, d_model]
# Reconstruction targets only the current point x_t, while the left side of
# early windows is padded by repeating the first observed point.
LOCAL_CONFIG_OVERRIDES = {
    ...
    # P comes from the previous real-time window X_{t-1}; geometry follows the
    # teacher-version raw latent objective with an absolute injected margin.
